...
class PlasTechEnv(gym.Env):
    ...
    def _generate_obstacles(self):
        obstacles = []
        while len(obstacles) < int(self.grid_size * self.grid_size * 0.2):
    ...
    def reset(self):
        ...
        self.visited_states = set([tuple(self.agent_position)])
    ...
    def _get_obs(self):
        self.state.fill(0)
        self.state[self.agent_position[0], self.agent_position[1]] = [1, 0, 0]  # Agent position
        self.state[self.goal_position[0], self.goal_position[1]] = [0, 1, 0]  # Goal position
        for obs in self.obstacles:
            self.state[obs[0], obs[1]] = [0, 0, 1]  # Obstacles
        return self.state
